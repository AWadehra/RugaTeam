...
RUGA integrates with [ActivePieces](https://www.activepieces.com), a powerful workflow automation platform that allows you to create automatic workflows for running AI agents and automations. ActivePieces is a sponsor of this hackathon.

![ActivePieces](assets/activepieces.png)

### Overview

You can create workflows in ActivePieces that are triggered by RUGA's `/chat` endpoint. This enables powerful automation scenarios, such as automatically generating and sending reports via email based on document analysis.

### Example Workflow: Automated Report Generation & Email

We've created a sample workflow that:

1. **Receives a report** generated by RUGA's `/chat` endpoint via webhook
2. **Processes the report** using an LLM to format it as an email
3. **Sends the email** to different people or groups automatically

### How It Works

1. **Set up ActivePieces workflow:**
   - Create a new workflow in ActivePieces
   - Add a webhook trigger to receive data from RUGA
   - Add an LLM step to format the report as an email
   - Add an email step to send to recipients

2. **Configure RUGA chat to trigger workflow:**
   - The AI agent in RUGA's `/chat` endpoint can use webhooks
   - Point the webhook to your ActivePieces endpoint
   - When the chat generates a report, it automatically triggers the workflow

3. **Automated execution:**
   - User chats with documents: `ruga chat "Generate a summary report of all research documents"`
   - RUGA generates the report via LLM
   - Report is sent to ActivePieces webhook
   - ActivePieces workflow formats and emails the report
   - Recipients receive the formatted email automatically

### Setting Up the Integration

1. **Create ActivePieces workflow:**
   - Sign up at [activepieces.com](https://www.activepieces.com)
   - Create a new workflow with a webhook trigger
   - Configure the webhook URL (you'll use this in step 3)

2. **Add workflow steps:**
   - **LLM Step**: Format the report content into a professional email
   - **Email Step**: Configure recipients (can be dynamic based on report content)
   - **Conditional Logic**: Route to different groups based on report type

3. **Configure RUGA chat webhook:**
   - When using the `/chat` endpoint, the AI agent can call webhooks
   - Point the webhook to your ActivePieces endpoint URL
   - The chat response will include instructions to trigger the workflow

### Example Use Cases

- **Automated Weekly Reports**: Chat generates weekly summaries, automatically emailed to team
- **Document Analysis Alerts**: When specific topics are found, automatically notify relevant stakeholders
- **Research Distribution**: Automatically distribute research findings to different groups based on categories
- **Meeting Summaries**: Generate and email meeting summaries from analyzed documents

### Benefits

- ✅ **Automation**: No manual email sending required
- ✅ **Intelligent Routing**: Send to different groups based on content
- ✅ **Professional Formatting**: LLM ensures emails are well-formatted
- ✅ **Scalable**: Handle multiple reports and recipients automatically
- ✅ **Integration**: Seamlessly connects RUGA's AI capabilities with email workflows

### Resources

- [ActivePieces Documentation](https://www.activepieces.com/docs)
- [ActivePieces Webhook Guide](https://www.activepieces.com/docs/triggers/webhook)
- [RUGA Chat API Documentation](#server-api)
